...
LTCLAW-GY.X is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

LTCLAW-GY.X supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/ltclaw-gy-x/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in LTCLAW-GY.X.
...
Channels are how LTCLAW-GY.X talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so LTCLAW-GY.X can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/ltclaw-gy-x/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/ltclaw-gy-x/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `ltclaw-gy-x channels install <key>` — create a template or copy from `--path` / `--url`
  - `ltclaw-gy-x channels add <key>` — install and add to config
  - `ltclaw-gy-x channels remove <key>` — remove custom channel from `custom_channels/`
  - `ltclaw-gy-x channels config` — interactive config
...
**Skills** define what LTCLAW-GY.X can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/ltclaw-gy-x/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** LTCLAW-GY.X supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
LTCLAW-GY.X aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `ltclaw-gy-x init` / `ltclaw-gy-x app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** LTCLAW-GY.X supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
- **Documentation:** Fixes and improvements to [the docs](https://ltclaw-gy-x.agentscope.io/) (under `website/public/docs/`) and README are always welcome.
